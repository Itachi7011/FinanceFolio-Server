...
const NewUserRegistrationSchema = new mongoose.Schema({
  ...
  // Preferences - REPLACE your existing budgetSettings with this expanded version
  settings: {
...
// Add these indexes after schema definition but before model creation
NewUserRegistrationSchema.index({ email: 1 }, { unique: true });
NewUserRegistrationSchema.index({ userName: 1 }, { unique: true });
NewUserRegistrationSchema.index({ status: 1 });
NewUserRegistrationSchema.index({ createdAt: -1 });
...
NewUserRegistrationSchema.methods.comparePassword = async function(candidatePassword) {
